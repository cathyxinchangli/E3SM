  ...
  !-----------------------------------------------------------------------
   ...
   do l = bounds%begl,bounds%endl
      if (lun_pp%urbpoi(l)) then
         ...
         ip = mct_aVect_indexRA(this%sdat_urbantv%avs(1),trim(stream_var_name(lun_pp%itype(l))))
         !
   ...
   do l = bounds%begl,bounds%endl
      if (lun_pp%urbpoi(l)) then
         ...
         if ( .not. urban_valid(g,t) .or. (this%t_building_max(l) <= 0._r8)) then
            found = .true.
            gindx = g
            tindx = t
            lindx = l
   ...
   if ( found ) then
      ...
      write(iulog,*)'urban_valid:     ',urban_valid(gindx,tindx)
